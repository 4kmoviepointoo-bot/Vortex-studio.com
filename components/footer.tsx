...
function FooterLink({ href, label, external = false }: { href: string; label: string; external?: boolean }) {
  const content = (
    <span className="group inline-flex items-center gap-1.5 text-sm text-slate-400 hover:text-emerald-400 transition-colors duration-200 py-1">
      {label}
    </span>
  )
    ...
    <footer className="bg-slate-950 text-slate-400 border-t border-slate-800/80 pt-16 pb-12">
      <div className="max-w-6xl mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
          <div>
            <h3 className="text-lg font-bold text-white mb-3">Vortex Studio <span className="text-emerald-400 font-normal text-xs bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20">v1</span></h3>
